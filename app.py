...
def shelflife_atualizar():
    ...
    # data_inconsistencia: None = não enviar (preserva o valor do banco)
    # string vazia "" = limpar intencionalmente
    # "yyyy-mm-dd" = atualizar
    _data_inc_raw = data.get('data_inconsistencia')
    _data_inc_val = None if not _data_inc_raw else _data_inc_raw
    ...
    cursor.execute("""
        UPDATE shelflife SET
            ...
            venda_3meses         = %s,
            venda_mes            = %s,
            data_inconsistencia  = CASE WHEN %s IS NOT DISTINCT FROM NULL
                                        THEN data_inconsistencia
                                        ELSE %s::date END,
            obs_logistica        = %s,
            obs_gerais           = %s,
            acao                 = %s,
