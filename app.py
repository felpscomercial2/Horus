...
def pivot_clientes_novo():
    """
    Schema confirmado:
    - carteira:    cod_vendedor (codigo)
    - faturamento: cod_vendedor (mesmo codigo) + vendedor (nome)
    
    Lógica de filtros:
    - Sem filtro          : todos os clientes da carteira (LEFT JOIN faturamento)
    - Vendedor selecionado: busca cod_vendedor pelo NOME em faturamento,
                            filtra carteira por cod_vendedor → todos os clientes daquele vendedor
    - Produto selecionado : apenas clientes que compraram o produto (INNER JOIN)
    - Produto + Vendedor  : clientes que compraram o produto via aquele vendedor
    - Períodos (anos)     : filtro de faturamento, clientes sempre aparecem todos
    """
    ...
    # Monta filtro de período para o LEFT JOIN de faturamento
    # Aceita 'periodo' no formato 'yyyy-mm' (preciso) ou 'ano' (apenas ano)
    periodos = request.args.getlist('periodo')  # ex: ['2026-04', '2026-03']
    ano_filter = ''
    ano_params = []
    if periodos:
        # Filtra por ano+mes exatos
        conds = []
        for per in periodos:
            try:
                ano_p, mes_p = per.split('-')
                conds.append('(f.ano = %s AND f.mes = %s)')
                ano_params += [int(ano_p), int(mes_p)]
            except:
                pass
        if conds:
            ano_filter = 'AND (' + ' OR '.join(conds) + ')'
    elif anos:
    ...
    if produtos:
        # COM produto: INNER JOIN — só clientes que compraram o produto
        # Vendedor filtra por nome direto em faturamento (tem o campo 'vendedor')
        ...
        # SEM produto: base é a tabela carteira (garante que clientes sem compra apareçam)
        # 1. Resolve nomes de vendedores → cod_vendedor via faturamento
        # 2. Filtra carteira por cod_vendedor
        # 3. LEFT JOIN com faturamento para pegar valores (ou NULL se não comprou)
        ...
        if vendedores:
            ...
            cod_rows = consultar(
                f"SELECT DISTINCT cod_vendedor FROM faturamento "                f"WHERE vendedor IN ({vend_ph}) AND cod_vendedor IS NOT NULL AND cod_vendedor != ''",
                vendedores
            )
            cods = [r['cod_vendedor'] for r in cod_rows]
            if cods:
                cod_ph      = ','.join(['%s'] * len(cods))
                cart_filter = f'WHERE c.cod_vendedor IN ({cod_ph})'
                cart_params = cods
            else:
                # Nenhum cod_vendedor encontrado — retorna vazio para não mostrar tudo
                cursor.close(); conn.close()
                return jsonify([])

        # Query 1: todos os clientes da carteira (sem filtro de período)
        # O filtro de período só afeta os VALORES, não a lista de clientes
        cursor.execute(f"""
            SELECT
                c.cliente,
                c.cod_cliente,
            ...
            FROM carteira c
            LEFT JOIN (
                SELECT DISTINCT cod_vendedor, vendedor
                FROM faturamento
