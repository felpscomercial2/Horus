...
def shelflife_upload():
    """Recebe JSON com os dados da planilha e salva no banco"""
    ...
    # Remove registros da mesma semana e unidade antes de inserir
    cursor.execute(
        "DELETE FROM shelflife WHERE semana = %s AND unidade = %s",
        [semana, unidade]
    )
    ...
    for p in produtos:
        ...
        if validade:
            try:
                ...
                val_date = datetime.strptime(validade[:10], '%Y-%m-%d').date()
        ...
        # Detecta se é código SL
        nome = p.get('produto', '')
        is_sl = 'SL' in nome.upper()[:5]

        cursor.execute("""
            INSERT INTO shelflife (
                semana, cod_produto, produto, marca, unidade,
                quantidade_log, validade, dias_vencimento, status, is_sl
        ...
        """, [
            ...
            unidade,
            p.get('quantidade'),
            validade[:10] if validade else None,
            dias,
            status,
            is_sl
        ])
...
def shelflife_listar():
    """Lista produtos da semana mais recente ou da semana especificada"""
    ...
    resultado = consultar(f"""
        SELECT * FROM shelflife {where_str}
        ORDER BY dias_vencimento ASC, status ASC
    """, params)
...
def shelflife_atualizar():
    """Atualiza quantidade atual, ação e observação de um produto"""
    data = request.get_json()
    id_prod = data.get('id')
...
@app.route('/api/shelflife/vendas')
def shelflife_vendas():
    """Busca vendas dos últimos 3 meses e do mês atual para um produto"""
    cod = request.args.get('cod_produto')
    if not cod:
        return jsonify({})
    ...
    # 3 meses atrás
    mes3 = mes_atual - 3 if mes_atual > 3 else mes_atual - 3 + 12
    ano3 = ano_atual if mes_atual > 3 else ano_atual - 1

    resultado = consultar("""
        SELECT
            ROUND(CAST(SUM(CASE
                WHEN (ano > %s OR (ano = %s AND mes >= %s))
                AND tipo_operacao = 'Venda' THEN valor_nf ELSE 0 END) AS NUMERIC), 2) AS venda_3meses,
            ROUND(CAST(SUM(CASE
                WHEN ano = %s AND mes = %s
                AND tipo_operacao = 'Venda' THEN valor_nf ELSE 0 END) AS NUMERIC), 2) AS venda_mes,
            ROUND(CAST(SUM(CASE
                WHEN (ano > %s OR (ano = %s AND mes >= %s))
                AND tipo_operacao = 'Venda' THEN quantidade ELSE 0 END) AS NUMERIC), 0) AS qtd_3meses
        FROM faturamento
        WHERE cod_produto = %s
    """, [ano3, ano3, mes3, ano_atual, mes_atual, ano3, ano3, mes3, cod])
