...
def filtros():
    ...
    # Uma única conexão, 7 queries em paralelo com subqueries
    conn   = get_conn()
    cursor = conn.cursor()
    cursor.execute("""
        SELECT
            ARRAY(SELECT DISTINCT ano  FROM faturamento WHERE ano  IS NOT NULL AND ano > 0 ORDER BY ano DESC)  AS anos,
            ARRAY(SELECT DISTINCT mes  FROM faturamento WHERE mes  IS NOT NULL AND mes > 0 ORDER BY mes)       AS meses,
            ARRAY(SELECT DISTINCT unidade FROM faturamento WHERE unidade IS NOT NULL ORDER BY unidade)          AS unidades,
            ARRAY(SELECT DISTINCT uf   FROM faturamento WHERE uf   IS NOT NULL AND uf != '' ORDER BY uf)       AS ufs,
            ARRAY(SELECT DISTINCT marca FROM faturamento WHERE marca IS NOT NULL ORDER BY marca)                AS marcas,
            ARRAY(SELECT DISTINCT tipo_operacao FROM faturamento WHERE tipo_operacao IS NOT NULL ORDER BY tipo_operacao) AS tipos,
            ARRAY(SELECT DISTINCT vendedor FROM faturamento WHERE vendedor IS NOT NULL ORDER BY vendedor)        AS vendedores
    """)
    row = cursor.fetchone()
    ...
    resultado = {
        'anos':       list(row[0] or []),
        'meses':      list(row[1] or []),
        'unidades':   list(row[2] or []),
        'ufs':        list(row[3] or []),
        'marcas':     list(row[4] or []),
        'tipos':      list(row[5] or []),
        'vendedores': list(row[6] or []),
    }
...
def pivot_clientes_novo():
    try:
        ...
        if periodos:
            ...
            ano_filter = 'AND f.ano IN (' + ano_ph + ')'
        ...
        if produtos:
            ...
            cursor.execute(sql, ['Venda','Devolucao','Venda','Devolucao'] + params)
            ...
            if vendedores:
                ...
                if cods:
                    ...
                    cart_filter = 'WHERE c.cod_vendedor IN (' + cod_ph + ')'
                    cart_params = cods
                else:
                    cursor.close(); conn.close()
                    return jsonify([])
            ...
            cursor.execute(sql, ['Venda','Devolucao','','Venda','Devolucao'] + ano_params + cart_params)
