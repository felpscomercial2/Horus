...
#   X-API-Key: <valor da variável de ambiente API_KEY>
# Rotas públicas (sem verificação): / e /ping
# ============================================================

def verificar_chave():
    """
    Retorna None se a chave for válida.
    Retorna uma Response de erro (401 ou 500) se inválida.
    """
    api_key = os.environ.get('API_KEY')
    if not api_key:
        return jsonify({"erro": "API_KEY não configurada no servidor"}), 500

    chave_enviada = request.headers.get('X-API-Key')
    if not chave_enviada or chave_enviada != api_key:
...
CAMPOS_PERMITIDOS = {'ano', 'mes', 'unidade', 'uf', 'tipo_operacao', 'marca', 'vendedor'}
