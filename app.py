...
def shelflife_atualizar():
    ...
    # Registra no histórico — só se tiver ação ou observação
    acao     = data.get('acao','')
    obs_log  = data.get('obs_logistica','')
    obs_ger  = data.get('obs_gerais','')
    vendedor = data.get('vendedor','')
    if acao or obs_log or obs_ger or vendedor:
        cursor.execute("""
        ...
        if row:
            cursor.execute("""
                INSERT INTO shelflife_historico (
                    ...
                    acao, vendedor, obs_logistica, obs_gerais, data_inconsistencia
