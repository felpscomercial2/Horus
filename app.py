...
# Compatível com Flask 2.x e 3.x
...
# Flask 2.2 e anteriores usam app.json_encoder
# Flask 2.3+ usa app.json.provider ou patch direto no jsonify
try:
    app.json_encoder = DateEncoder  # Flask <= 2.2
except AttributeError:
    pass

# Patch compatível com todas as versões: converte datas antes de serializar
_original_jsonify = jsonify
def _safe_jsonify(data, **kwargs):
    def convert(obj):
        ...
        if isinstance(obj, (datetime, _date)):
            return obj.isoformat()
        return obj
    return _original_jsonify(convert(data), **kwargs)

# Substituímos jsonify globalmente
import flask as _flask
_flask.jsonify = _safe_jsonify
jsonify = _safe_jsonify
